-- CE-OS PostgreSQL Schema (self-hosted)
-- Monetary values: BIGINT UGX (smallest unit; never floats) unless column ends in _usd.
--   BIGINT (not INTEGER) so large/aggregate amounts can't overflow ~2.1B.
-- Weights: NUMERIC kg.  Percentages: NUMERIC.  PKs: UUID via gen_random_uuid().
-- All user-facing FKs reference users(id).
--
-- AUTHZ NOTE: role enforcement lives in the API layer (Fastify middleware),
-- mirroring ROLE_PERMISSIONS in src/core/auth.tsx — NOT Postgres RLS, because the
-- app connects as a single DB role. If you ever expose the DB directly to multiple
-- roles (e.g. PostgREST), add RLS policies then. Keep all writes going through the
-- API so the audit_log and computed fields stay authoritative.
--
-- gen_random_uuid() requires pgcrypto on older PG; PG13+ has it built in.
...
-- ─────────────────────────────────────────────
-- LOOKUP TABLES (managed lists)
-- ─────────────────────────────────────────────

CREATE TABLE coffee_grades (
  id        SERIAL PRIMARY KEY,
  name      TEXT NOT NULL UNIQUE,
  sellable  BOOLEAN NOT NULL DEFAULT true,  -- false for 'Off-grade / Reject'
...
CREATE TABLE districts (
  id    SERIAL PRIMARY KEY,
  name  TEXT NOT NULL UNIQUE,
  code  TEXT NOT NULL UNIQUE   -- e.g. 'MBL', 'SIP' (used in batch_code generation)
);
...
-- Seed data — reconciled with the frontend managed lists (src/core/settings.ts)
-- and the rule-based grader (src/shared/calc/quality.ts recommendGrade), which can
-- return 'Off-grade / Reject'. That row MUST exist so quality_metrics.recommended_grade_id
-- never violates its FK.
...
CREATE TABLE users (
  ...
  -- argon2/bcrypt hash; nullable so users can be invited before setting a password
  password_hash  TEXT,
  active         BOOLEAN NOT NULL DEFAULT true,
  last_login_at  TIMESTAMPTZ,
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by     UUID REFERENCES users(id),   -- NULL for the bootstrap admin
...
CREATE TABLE batches (
  ...
  net_payable_weight_kg  NUMERIC,           -- computed at grading, persisted
  clean_output_kg        NUMERIC,           -- set after processing
  buyer_id               UUID REFERENCES clients(id),
  market_price_per_kg    BIGINT NOT NULL,   -- UGX/kg agreed with farmer
  created_at             TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by             UUID NOT NULL REFERENCES users(id),
  updated_at             TIMESTAMPTZ,
...
-- ─────────────────────────────────────────────
-- QUALITY METRICS (one row per batch)
-- ─────────────────────────────────────────────
...
CREATE TABLE quality_metrics (
  ...
  -- defect breakdown (atomic columns, not a nested object)
  ...
  -- 'Off-grade / Reject' is a real row in coffee_grades, so this FK always resolves
...
-- ─────────────────────────────────────────────
-- ALLOCATION GROUPS
-- Groups expense lines that are shared across multiple batches
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- EXPENSE LINES
-- batch_id NULL allowed for a pure allocation-group line; non-null ties to one batch
-- ─────────────────────────────────────────────
...
CREATE TABLE expense_lines (
  ...
  -- an allocated line must belong to a group; a per_kg line must not
...
-- ─────────────────────────────────────────────
-- FOREX SNAPSHOTS
-- batch_id NULL = global live ticker; non-null = rate locked to a batch.
-- USD/UGX today; when generalising to EUR/GBP add a currency_pair column and a
-- rate stored as NUMERIC for sub-unit precision.
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- CONTAINER ALLOCATIONS (batch → shipment)
-- ─────────────────────────────────────────────
...
CREATE TABLE notifications (
  ...
  entity_type  TEXT,     -- 'batch', 'shipment', 'expense', etc.
  entity_id    UUID,     -- the referenced row's id
  read         BOOLEAN NOT NULL DEFAULT false,
  created_at   TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by   UUID NOT NULL REFERENCES users(id),
  updated_at   TIMESTAMPTZ   -- tracks when read status changed
);
...
CREATE TABLE audit_log (
  ...
  entity_type TEXT NOT NULL,   -- 'batch', 'expense', 'shipment', etc.
...
-- Defaults reconciled with src/core/settings.ts
-- ─────────────────────────────────────────────
